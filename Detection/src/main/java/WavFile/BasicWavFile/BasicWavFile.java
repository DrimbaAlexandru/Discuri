...
// Version 1.0
package WavFile.BasicWavFile;

import java.io.*;

public class BasicWavFile
{
	private enum IOState {CLOSED, RW};
	...
	// Cannot instantiate BasicWavFile directly, must either use newWavFile() or openWavFile()
	private BasicWavFile()
	{
	...
	public static BasicWavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, BasicWavFileException
	{
		// Instantiate new Wavfile and initialise
		BasicWavFile wavFile = new BasicWavFile();
		wavFile.file = file;
        ...
        wavFile.header_size = 0;

		// Sanity check arguments
		if (numChannels < 1 || numChannels > 65535) throw new BasicWavFileException("Illegal number of channels, valid range 1 to 65536");
		if (numFrames < 0) throw new BasicWavFileException("Number of frames must be positive");
		if (validBits < 2 || validBits > 65535) throw new BasicWavFileException("Illegal number of valid bits, valid range 2 to 65536");
		if (sampleRate < 0) throw new BasicWavFileException("Sample rate must be positive");

		// Create output stream for writing data
		wavFile.IOStream = new RandomAccessFile(file,"rwd");
		...
		wavFile.IOStream.write(wavFile.buffer, 0, 12);
		...
		// Write Format Chunk
		wavFile.IOStream.write(wavFile.buffer, 0, 24);
		...
		// Write Format Chunk
		wavFile.IOStream.write(wavFile.buffer, 0, 8);
		...
		wavFile.ioState = IOState.RW;

		wavFile.header_size = wavFile.IOStream.getFilePointer();

		return wavFile;
	}

	public static BasicWavFile openWavFile(File file) throws IOException, BasicWavFileException
	{
		...
		int bytesRead = wavFile.IOStream.read(wavFile.buffer, 0, 12);
		if (bytesRead != 12) throw new BasicWavFileException("Not enough wav file bytes for header");
		...
		if (riffChunkID != RIFF_CHUNK_ID) throw new BasicWavFileException("Invalid Wav Header data, incorrect riff chunk ID");
		if (riffTypeID != RIFF_TYPE_ID) throw new BasicWavFileException("Invalid Wav Header data, incorrect riff type ID");

		// Check that the file size matches the number of bytes listed in header
		if (file.length() != chunkSize+8) {
			throw new BasicWavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
		}
		...
		while (true)
		{
			...
			bytesRead = wavFile.IOStream.read(wavFile.buffer, 0, 8);
			if (bytesRead == -1) throw new BasicWavFileException("Reached end of file without finding format chunk");
			if (bytesRead != 8) throw new BasicWavFileException("Could not read chunk header");
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				bytesRead = wavFile.IOStream.read(wavFile.buffer, 0, 16);

				// Check this is uncompressed data
				int compressionCode = (int) getLE(wavFile.buffer, 0, 2);
				if (compressionCode != 1) throw new BasicWavFileException("Compression Code " + compressionCode + " not supported");
				...
				if (wavFile.numChannels == 0) throw new BasicWavFileException("Number of channels specified in header is equal to zero");
				if (wavFile.blockAlign == 0) throw new BasicWavFileException("Block Align specified in header is equal to zero");
				if (wavFile.validBits < 2) throw new BasicWavFileException("Valid Bits specified in header is less than 2");
				if (wavFile.validBits > 64) throw new BasicWavFileException("Valid Bits specified in header is greater than 64, this is greater than a long can hold");

				// Calculate the number of bytes required to hold 1 sample
				wavFile.bytesPerSample = (wavFile.validBits + 7) / 8;
				if (wavFile.bytesPerSample * wavFile.numChannels != wavFile.blockAlign)
					throw new BasicWavFileException("Block Align does not agree with bytes required for validBits and number of channels");

				// Account for number of format bytes and then skip over
				// any extra format bytes
				numChunkBytes -= 16;
				if (numChunkBytes > 0) wavFile.IOStream.skipBytes((int)numChunkBytes);
			}
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				if (foundFormat == false) throw new BasicWavFileException("Data chunk found before Format chunk");

				// Check that the chunkSize (wav data length) is a multiple of the
				// block align (bytes per frame)
				if (chunkSize % wavFile.blockAlign != 0) throw new BasicWavFileException("Data Chunk size is not multiple of Block Align");
			...
			else
			{
                ...
                wavFile.IOStream.skipBytes((int)numChunkBytes);
			}
		}

		// Throw an exception if no data chunk has been found
		if (foundData == false) throw new BasicWavFileException("Did not find a data chunk");
	...
	private void writeSample(long val) throws IOException
	{
		for (int b=0 ; b<bytesPerSample ; b++)
		{
			...
			{
                IOStream.write(buffer, 0, BUFFER_SIZE);
				bufferPointer = 0;
			}
	...
	private long readSample() throws IOException, BasicWavFileException
	{
		long val = 0;
	...
	// Integer
	// -------
	public int readFrames(int[] sampleBuffer, int numFramesToRead) throws IOException, BasicWavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public int readFrames(int[] sampleBuffer, int offset, int numFramesToRead) throws IOException, BasicWavFileException
	{
		if (ioState != IOState.RW) throw new IOException("Cannot read from BasicWavFile instance");
	...
	public int writeFrames(int[] sampleBuffer, int numFramesToWrite) throws IOException, BasicWavFileException
	{
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}

	public int writeFrames(int[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, BasicWavFileException
	{
		if (ioState != IOState.RW) throw new IOException("Cannot write to BasicWavFile instance");
	...
	// Long
	// ----
	public int readFrames(long[] sampleBuffer, int numFramesToRead) throws IOException, BasicWavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public int readFrames(long[] sampleBuffer, int offset, int numFramesToRead) throws IOException, BasicWavFileException
	{
		if (ioState != IOState.RW) throw new IOException("Cannot read from BasicWavFile instance");
	...
	public int writeFrames(long[] sampleBuffer, int numFramesToWrite) throws IOException, BasicWavFileException
	{
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}

	public int writeFrames(long[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, BasicWavFileException
	{
		if (ioState != IOState.RW) throw new IOException("Cannot write to BasicWavFile instance");
	...
	// Double
	// ------
	public int readFrames(double[] sampleBuffer, int numFramesToRead) throws IOException, BasicWavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public int readFrames(double[] sampleBuffer, int offset, int numFramesToRead) throws IOException, BasicWavFileException
	{
		if (ioState != IOState.RW) throw new IOException("Cannot read from BasicWavFile instance");
	...
	public int writeFrames(double[] sampleBuffer, int numFramesToWrite) throws IOException, BasicWavFileException
	{
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}

	public int writeFrames(double[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, BasicWavFileException
	{
		if (ioState != IOState.RW) throw new IOException("Cannot write to BasicWavFile instance");
	...
	public void close() throws IOException
	{
		if (IOStream != null)
		{
			...
			if (bufferPointer > 0) IOStream.write(buffer, 0, bufferPointer);

			// If an extra byte is required for word alignment, add it to the end
			if (wordAlignAdjust) IOStream.write(0);
	...
	public void set_stream_position( long samples_to_skip, int channel ) throws IOException
    {
