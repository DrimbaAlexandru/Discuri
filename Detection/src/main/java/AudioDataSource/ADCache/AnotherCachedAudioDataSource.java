 ...
 */
public class AnotherCachedAudioDataSource implements IAudioDataSource
{
    ...
    public AnotherCachedAudioDataSource( IAudioDataSource dataSource, int max_cached_samples, int max_cache_page_size )
    {
    ...
    private AudioSamplesWindow get_cached_page( int any_sample_in_page ) throws DataSourceException
    {
        ...
        if( win == null )
        {
            ...
            if( win.get_capacity() < cache_page_size )
            {
                ...
                win = new AudioSamplesWindow( samples, win.get_first_sample_index(), win.get_length(), win.get_channel_number() );
            }
            cache_window( win );
